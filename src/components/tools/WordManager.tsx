        ...
        <div className="w-full lg:w-[280px] shrink-0 flex flex-col gap-8 relative z-20 lg:h-full">
          <div className="bg-slate-50 p-6 rounded-[2rem] border-4 border-white flex flex-col gap-6 relative overflow-visible lg:overflow-hidden shrink-0 lg:h-full">
             <div className="flex items-center justify-between lg:justify-start gap-4 relative z-10">
                ...
                <div className="lg:hidden relative flex-1 max-w-[200px]">
                  <select
                    ...
                    className="appearance-none w-full bg-surface pl-4 pr-10 py-2.5 rounded-xl border-2 border-slate-100 text-slate-700 font-black text-[10px] uppercase tracking-widest outline-none focus:ring-2 focus:ring-indigo-500/20"
                  >
             ...
             <div className="space-y-2 relative z-10 flex-1 overflow-y-auto no-scrollbar pr-2">
               <span className="text-[9px] font-black text-primary/70 uppercase tracking-[0.4em] ml-1">Grade Lists</span>
                   ...
                   <button
                     ...
                     className={`w-full h-12 rounded-xl border-2 flex items-center justify-between px-4 transition-all ${
                       selectedYear === year 
                         ...
                         : 'bg-surface border-transparent text-slate-600 hover:bg-slate-100'
                     }`}
                   >
                     <div className="flex items-center gap-2">
                        <div className={`w-6 h-6 rounded-md flex items-center justify-center text-[10px] font-black ${selectedYear === year ? 'bg-surface/20 text-white' : 'bg-slate-200 text-slate-500'}`}>
                          {year}
                        </div>
             ...
             <button
               ...
               className="w-full h-16 rounded-xl border-4 border-white bg-surface text-slate-500 hover:bg-slate-100 transition-all flex flex-row items-center justify-center gap-4 relative z-10 mt-auto mb-2"
             >
             ...
             <button
               ...
               className={`w-full h-16 rounded-xl border-4 transition-all flex flex-row items-center justify-center gap-4 relative z-10 ${
                 selectedYear === 'saved'
                   ...
                   : 'bg-surface text-slate-500 border-white hover:bg-slate-100'
               }`}
             >
            ...
            <div className="flex-1 overflow-auto no-scrollbar relative z-10 rounded-[2rem]">
               <table className="w-full text-left border-separate border-spacing-0">
                  <thead className="sticky top-0 z-20">
                     <tr className="bg-surface/80 backdrop-blur-md border-b-4 border-white overflow-hidden rounded-t-[2rem]">
                 ...
                 <tbody className="divide-y-2 divide-white">
                    {currentData.map((item: any, idx: number) => {
                      ...
                      return (
                        <tr key={item.id} className="group transition-all hover:bg-surface">
                          ...
                          <td className="px-4 py-2">
                            <div className="flex flex-col gap-1">
                              <span className="text-sm font-black text-slate-800 group-hover:text-primary transition-colors uppercase tracking-tight italic leading-none">
                                {item.name}
                              </span>
                            </div>
                          </td>
                            ...
                            <div className="flex flex-wrap gap-1">
                              ...
                              {item.words.map((word: string, i: number) => {
                                ...
                                return (
                                  <span key={i} className={`px-2 py-1 border-2 rounded-xl text-[10px] font-black uppercase tracking-widest ${
                                    isAdded 
                                      ? 'bg-success-bg border-emerald-200 text-success' 
                                      : 'bg-surface border-slate-100 text-neutral-400'
                                  }`}>
                                    {word}
                                  </span>
                                );
                              ...
                              {!isCurriculumView && sourceWords && sourceWords.map((word: string, i: number) => {
                                if (!item.words.includes(word)) {
                                  return (
                                    <span key={`rem-${i}`} className="px-2 py-1 bg-caution-bg border-2 border-rose-200 rounded-xl text-[10px] font-black text-rose-400 uppercase tracking-widest line-through">
                                      {word}
                                    </span>
                                  );
                                }
                          ...
                          <td className="pr-6 py-2 text-right">
                            <div className="flex items-center justify-end gap-2 transition-all opacity-100">
                              {isCurriculumView ? (
                                <>
                                  <button onClick={() => addToList(item)} disabled={isAlreadyAdded} className={`w-8 h-8 flex items-center justify-center rounded-xl transition-all ${isAlreadyAdded ? 'bg-emerald-500 text-white' : 'bg-primary text-white hover:bg-primary/90'} disabled:opacity-30`} title="Add">
                                    {isAlreadyAdded ? <Check size={16} strokeWidth={4} /> : <Plus size={16} strokeWidth={4} />}
                                  </button>
                                </>
                              ) : (
                                <>
                                  <button onClick={() => handleEditClick(item, false)} className="w-8 h-8 flex items-center justify-center text-slate-300 hover:text-primary bg-surface rounded-xl border-2 border-slate-50 transition-all" title="Edit">
                                    <Pencil size={16} strokeWidth={3} />
                                  </button>
                                  <button onClick={() => handleDeleteCustom(item.id)} className="w-8 h-8 flex items-center justify-center text-slate-300 hover:text-caution bg-surface rounded-xl border-2 border-slate-50 transition-all" title="Delete">
              ...
              {currentData.length === 0 && (
                <div className="flex flex-col items-center justify-center p-40 text-center gap-8">
                  <div className="w-24 h-24 bg-surface rounded-[2.5rem] flex items-center justify-center border-4 border-slate-50">
        ...
        {editingList && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 lg:p-6 bg-slate-950/60 backdrop-blur-xl italic">
            <motion.div
              ...
              className="bg-surface rounded-[2.5rem] lg:rounded-[4rem] p-6 lg:p-16 w-full max-h-full lg:h-auto max-w-2xl flex flex-col gap-6 lg:gap-10 relative overflow-hidden border-4 lg:border-8 border-white shadow-2xl"
            >
              ...
              <div className="flex justify-between items-start relative z-10 shrink-0">
                 <div className="flex items-center gap-4 lg:gap-6">
                    <div className="w-12 h-12 lg:w-16 lg:h-16 rounded-[1.25rem] lg:rounded-[1.5rem] bg-primary flex items-center justify-center text-white shrink-0">
                    ...
                    <div className="flex flex-col">
                       ...
                       <span className="text-[9px] lg:text-[11px] font-black text-neutral-400 uppercase tracking-[0.4em] mt-2 lg:mt-3">{editingList?.isNew ? 'Add your own words' : 'Changing words'}</span>
                    </div>
                 </div>
              ...
              <div className="flex flex-col flex-1 space-y-6 lg:space-y-8 relative z-10 min-h-0">
                <div className="space-y-3 lg:space-y-4 shrink-0">
                  <label className="text-[10px] font-black text-neutral-400 uppercase tracking-[0.4em] ml-2">Name</label>
                  <input
                    type="text"
                    value={editingList.name}
                    onChange={(e) => setEditingList({...editingList, name: e.target.value})}
                    className="w-full h-16 lg:h-20 px-4 bg-slate-50 border-4 border-transparent rounded-2xl lg:rounded-[1.5rem] text-sm font-black text-slate-800 outline-none focus:bg-surface focus:border-primary/20 transition-all uppercase tracking-widest"
                    placeholder="e.g., Spelling Week 1"
                  />
                </div>

                <div className="flex flex-col flex-1 space-y-3 lg:space-y-4 min-h-0">
                  <label className="text-[10px] font-black text-neutral-400 uppercase tracking-[0.4em] ml-2">Words (comma separated)</label>
                  <textarea
                    value={editingList.words}
                    onChange={(e) => setEditingList({...editingList, words: e.target.value})}
                    className="w-full flex-1 bg-slate-50 border-4 border-transparent rounded-[2rem] lg:rounded-[2.5rem] p-6 lg:p-10 text-lg lg:text-xl font-black text-slate-600 outline-none focus:bg-surface focus:border-primary/20 transition-all resize-none no-scrollbar italic leading-relaxed"
              ...
              <div className="flex gap-4 relative z-10 shrink-0">
                <button
                  ...
                  className="flex-1 h-16 lg:h-20 rounded-2xl lg:rounded-[2rem] text-[10px] lg:text-[11px] font-black text-neutral-400 uppercase tracking-[0.3em] hover:bg-slate-50 transition-all"
                >
                  Cancel
                </button>
                <button
                  onClick={handleSaveEdit}
                  className="flex-1 h-16 lg:h-20 bg-primary text-white rounded-2xl lg:rounded-[2rem] text-[10px] lg:text-sm font-black uppercase tracking-[0.3em] hover:bg-primary/90 transition-all active:scale-95 border-4 lg:border-8 border-white/5"
                >
                  Save
                </button>
              </div>
