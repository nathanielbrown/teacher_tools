...
export const HistoryPanel = ({
  ...
  const defaultRenderItem = (item: any, index: number) => (
    <div key={item.id || index} className="p-4 bg-white rounded-2xl border-2 border-slate-50  flex flex-col gap-1">
      <span className="text-sm font-black text-slate-700 uppercase tracking-tight">{item.title || item.content}</span>
      {(item.subtitle || item.subContent) && (
        <span className="text-[10px] font-black text-slate-300 tabular-nums">{item.subtitle || item.subContent}</span>
      )}
    </div>
  );
          ...
          <AnimatePresence initial={false} mode="popLayout">
            {items.length === 0 ? (
              <div className="h-full flex flex-col items-center justify-center py-8 bg-slate-50/50 border-2 border-slate-100 border-dashed rounded-[3rem]">
                ...
                <div className="text-[8px] font-black uppercase tracking-widest text-slate-300 text-center px-4 leading-relaxed">{emptyMessage}</div>
              </div>
            ) : (
