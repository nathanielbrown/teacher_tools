      ...
      <div className="flex items-center gap-2 md:gap-4 italic">
        {isMobile ? (
          <select
            ...
            className="bg-white border-2 border-indigo-100 text-indigo-600 px-4 py-2 rounded-xl font-bold text-xs outline-none uppercase tracking-widest"
          >
      ...
      <div className="w-full h-full flex flex-col gap-2 md:gap-4 min-h-0 relative z-10 p-2 md:p-6">
        <div className="flex-1 flex flex-col min-h-0 min-w-0 overflow-hidden">
           {view === 'countdowns' ? (
            ...
            >
              {/* Countdowns Header */}
              <div className="bg-white/80 backdrop-blur-xl p-4 md:p-6 rounded-[2rem] border-4 border-white flex flex-row items-center justify-between gap-4 shrink-0">
                 ...
                 <div className="flex items-center gap-4">
                   ...
                   <button 
                     ...
                     className={`bg-indigo-600 text-white rounded-[1.5rem] md:rounded-[2rem] font-bold uppercase tracking-widest hover:bg-indigo-700 transition-all flex items-center justify-center gap-2 active:scale-95 border-4 border-indigo-400 ${
                      ...
                      {events.map((event) => {
                        ...
                        if (countdownFormat === 'grid') {
                          return (
                            <motion.div
                              ...
                              className={`group relative bg-white/80 backdrop-blur-xl border-4 border-white rounded-[1.5rem] md:rounded-[2.5rem] p-3 md:p-8 flex flex-col transition-all ${timeLeft.passed ? 'opacity-40 grayscale' : ''}`}
                            >
                              <button
                                 onClick={() => removeEvent(event.id)}
                                 className="absolute top-2 right-2 md:top-4 md:right-4 text-slate-300 hover:text-rose-500 transition-all p-2 rounded-xl hover:bg-rose-50 border-2 border-transparent hover:border-rose-100 z-20"
                               >
                                 <Trash2 size={20} />
                               </button>
                              ...
                              <div className="flex-1 grid grid-cols-4 gap-2 md:gap-4 mb-2 md:mb-6">
                                {[
                                  ...
                                  <div key={i} className="flex flex-col items-center justify-center py-2 md:py-3 bg-slate-50 rounded-xl md:rounded-[1.5rem] border-2 md:border-4 border-white">
                                    <span className="text-xl md:text-2xl font-bold text-indigo-600 tabular-nums leading-none tracking-tighter">
                                      {t.v}
                                    </span>
                                    <span className="text-[8px] md:text-[9px] font-bold text-slate-400 uppercase tracking-widest mt-1 md:mt-2">
                        ...
                        return (
                          <motion.div
                            ...
                            animate={{ opacity: 1, x: 0 }}
                            exit={{ opacity: 0, x: -20 }}
                            className={`group relative bg-white/80 backdrop-blur-xl border-4 border-white rounded-[1.5rem] md:rounded-[2.5rem] p-3 md:p-6 flex flex-col md:flex-row items-start md:items-center justify-between gap-3 md:gap-0 transition-all ${timeLeft.passed ? 'opacity-40 grayscale' : ''}`}
                          >
                            <div className="flex flex-col gap-2 pl-2 md:pl-4">
                               ...
                               <span className="text-[10px] font-bold text-slate-400 uppercase tracking-[0.2em]">{dateString}</span>
                            </div>
              ...
              <div className="flex-1 bg-white/80 backdrop-blur-xl rounded-[4rem] overflow-hidden flex flex-col border-4 border-white relative min-h-0">
                ...
                <div className="p-4 flex-1 overflow-y-auto no-scrollbar relative z-10 italic">
                  <div className="grid grid-cols-7 gap-1 md:gap-2">
                    {DAY_NAMES.map(day => (
                      <div key={day} className="text-center py-2 md:py-4 text-[10px] font-bold text-slate-300 uppercase tracking-widest md:tracking-[0.5em]">{day}</div>
                    ))}
                            ...
                            >
                              <div className="w-full flex justify-between items-start mb-2">
                                ...
                                {isCurrentMonth && (
                                  <button
                                    ...
                                    className="p-1.5 bg-indigo-600 text-white rounded-lg opacity-100 md:opacity-0 md:group-hover:opacity-100 transition-all hover:bg-indigo-700 active:scale-90"
                                  >
                              ...
                              <div className="w-full flex flex-col gap-1.5 overflow-hidden">
                                {dayEvents.map((e: any, i: number) => (
                                  <div key={i} className="text-[8px] font-bold bg-indigo-600/10 text-indigo-700 rounded-lg px-2 md:px-3 py-1.5 truncate w-full text-left border border-indigo-100">
                                    {e.name}
                                  </div>
                                ))}
                              </div>
                ...
                {/* Footer Controls */}
                <div className="px-4 md:px-6 py-4 border-t-4 border-slate-50 bg-white flex items-center justify-end shrink-0 relative z-10">
                  <button 
                    onClick={() => { setShowAddModal(true); audioEngine.playTick(settings.soundTheme); }}
                    className="w-full md:w-auto px-6 py-3 md:px-10 md:py-4 bg-indigo-600 text-white rounded-[1.5rem] font-bold text-[10px] uppercase tracking-widest md:tracking-[0.3em] hover:bg-indigo-700 transition-all flex items-center justify-center gap-2 md:gap-3 active:scale-95 border-4 border-white/10"
                  >
          ...
          {(showCountdownAddModal || showAddModal) && (
            <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 bg-slate-800/80 backdrop-blur-xl">
               <motion.div 
                 ...
                 className="bg-white p-4 md:p-6 rounded-3xl max-w-lg w-full max-h-[85vh] overflow-y-auto space-y-4 relative border-8 border-white italic no-scrollbar shadow-2xl"
               >
                  <div className="absolute top-0 right-0 w-48 h-48 md:w-64 md:h-64 bg-indigo-50 rounded-full blur-[60px] md:blur-[80px] opacity-60 -z-10" />
                  
                  <button 
                     onClick={() => { setShowCountdownAddModal(false); setShowAddModal(false); audioEngine.playTick(settings.soundTheme); }} 
                     className="absolute top-6 right-6 md:top-12 md:right-12 p-2 md:p-3 text-slate-300 hover:text-slate-900 transition-all bg-slate-50 rounded-xl md:rounded-2xl"
                  >
                  ...
                  <form onSubmit={showCountdownAddModal ? handleAddCountdown : handleCalendarAddEvent} className="space-y-3">
                     <div className="space-y-1 text-left">
                        <label className="text-[10px] font-bold text-slate-300 uppercase tracking-widest ml-2">
                         ...
                         <input 
                           ...
                           className="w-full bg-slate-50 border-2 border-white p-3 md:p-4 rounded-xl md:rounded-2xl text-lg md:text-xl font-bold text-slate-900 outline-none focus:bg-white transition-all placeholder:text-slate-200"
                           required
                           autoFocus
                         />
                     </div>
                        ...
                        {showCountdownAddModal ? (
                           <div className="col-span-2 space-y-3 text-left">
                              <label className="text-[11px] font-bold text-slate-300 uppercase tracking-widest ml-4">
                               ...
                               <input 
                                 ...
                                 className="w-full bg-slate-50 border-2 border-white p-3 md:p-4 rounded-xl md:rounded-2xl text-base md:text-lg font-bold text-slate-900 outline-none focus:bg-white transition-all"
                                 required
                               />
                           </div>
                        ) : (
                           <>
                               <div className="space-y-1 text-left">
                                  ...
                                  <div className="w-full bg-indigo-600 p-3 md:p-4 rounded-xl md:rounded-2xl text-lg font-bold text-white border-2 border-indigo-400 text-center">
                     ...
                     <button
                       type="submit"
                       className="w-full py-4 bg-indigo-600 text-white rounded-2xl md:rounded-3xl font-bold uppercase tracking-[0.4em] text-xs md:text-sm hover:bg-indigo-700 transition-all active:scale-95 border-2 border-white/10"
                     >
