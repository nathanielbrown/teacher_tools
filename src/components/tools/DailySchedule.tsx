...
export const DailySchedule = () => {
  ...
  return (
    <ToolPanel className="font-['Outfit'] select-none italic" baseWidth={isMobile ? 600 : 1024} baseHeight={800} alignTop={true}>
      {/* Main Content Area */}
      <div className="w-full flex flex-col h-full relative z-10">

        {/* Simplified Title Section */}
        <div className="pt-2 pb-0 flex flex-col items-center shrink-0">
          <h2 className="text-4xl sm:text-5xl font-black text-black uppercase tracking-tighter italic leading-none">{currentDay}</h2>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-2 sm:p-4 no-scrollbar" ref={scrollRef}>
                    ...
                    return (
                      <React.Fragment key={item.id}>
                        ...
                        >
                          ...
                          {/* Activity Name */}
                          <div className="flex-1 min-w-0 flex items-start gap-3 pt-1">
                            <textarea
                              rows={1}
                              value={item.activity}
                              onChange={(e) => {
                              ...
                              className={`w-full text-sm font-semibold bg-transparent border-none outline-none focus:placeholder:opacity-0 italic text-black resize-none overflow-hidden py-0 leading-tight min-h-[1.25rem]`}
                            />
                             {status === 'current' && (
