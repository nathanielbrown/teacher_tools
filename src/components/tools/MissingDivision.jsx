...
import { Calculator, CheckCircle2, XCircle, RefreshCcw, ChevronRight, Star, Trophy, ArrowLeft, Divide } from 'lucide-react';
import confetti from 'canvas-confetti';
import { useSettings } from '../../contexts/SettingsContext';
import { audioEngine } from '../../utils/audio';

export const MissingDivision = () => {
  ...
  const generateQuestion = (max) => {
    ...
    if (max <= 10) {
      ...
      c = Math.floor(Math.random() * Math.floor(10 / b)) + 1;
    } else {
      b = Math.floor(Math.random() * 10) + 1;
      c = Math.floor(Math.random() * Math.floor(max / b)) + 1;
    }
    const a = b * c;
  ...
  if (gameState === 'menu') {
    return (
      <div className="max-w-4xl mx-auto h-full flex flex-col items-center justify-center p-4">
        ...
        >
          <div className="bg-emerald-500/10 w-24 h-24 rounded-3xl flex items-center justify-center mx-auto mb-6">
            <Divide size={48} className="text-emerald-600" />
          </div>
          <h1 className="text-5xl font-black text-gray-800 tracking-tight">Missing Division</h1>
          <p className="text-xl text-gray-500 font-medium max-w-md mx-auto">
            Test your knowledge of division with whole number answers.
          </p>
          
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 pt-4">
            <button
              onClick={() => startGame(10)}
              className="group relative p-8 bg-emerald-50 hover:bg-emerald-100 border-2 border-emerald-200 rounded-3xl transition-all hover:scale-105"
            >
              <div className="text-4xl mb-2">🍀</div>
              <div className="text-2xl font-black text-emerald-700">Numbers 1-10</div>
              <div className="text-emerald-600/60 font-bold">Perfect for starters</div>
            </button>
            <button
              onClick={() => startGame(100)}
              className="group relative p-8 bg-teal-50 hover:bg-teal-100 border-2 border-teal-200 rounded-3xl transition-all hover:scale-105"
            >
              <div className="text-4xl mb-2">🎋</div>
              <div className="text-2xl font-black text-teal-700">Numbers 1-100</div>
              <div className="text-teal-600/60 font-bold">The ultimate challenge</div>
            </button>
          </div>
        </motion.div>
      </div>
        ...
        >
          ...
          <div className="flex justify-center gap-2">
            {Array.from({ length: 10 }).map((_, i) => (
              <div 
                key={i} 
                className={`w-4 h-4 rounded-full ${i < score ? 'bg-emerald-500' : 'bg-gray-200'}`}
              />
            ))}
          </div>
          ...
          <div className="flex flex-col sm:flex-row gap-4 pt-4">
            ...
            <button
              ...
              className="flex-1 py-4 bg-emerald-600 text-white font-bold rounded-2xl hover:bg-emerald-700 transition-colors flex items-center justify-center gap-2 shadow-lg shadow-emerald-200"
            >
            ...
            {Array.from({ length: 10 }).map((_, i) => (
              <div 
                ...
                className={`w-3 h-3 rounded-full transition-colors ${
                  ...
                  i === currentIndex ? 'bg-emerald-500 animate-pulse' : 'bg-gray-200'
                }`}
              />
            ))}
          </div>
