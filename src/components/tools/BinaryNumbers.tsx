    ...
    setHeaderActions(
      isMobile ? (
        <select
          ...
          className="bg-slate-100 px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest text-primary outline-none border-none cursor-pointer appearance-none"
        >
        ...
        <div className="flex bg-slate-100 p-1 rounded-xl">
          <button
            ...
            className={`px-4 py-2 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all ${mode === 'practice' ? 'bg-surface text-primary shadow-sm' : 'text-neutral-400 hover:text-slate-600'
              }`}
          >
            Practice
          </button>
          <button
            ...
            className={`px-4 py-2 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all ${mode === 'challenge' ? 'bg-surface text-primary shadow-sm' : 'text-neutral-400 hover:text-slate-600'
              }`}
          >
            Challenge
          </button>
    ...
    >
      <div className={`w-full max-w-5xl mx-auto flex flex-col items-center gap-4 lg:gap-10 relative z-10 ${isMobile ? 'overflow-y-auto h-full px-2' : ''}`}>
          ...
          {mode === 'challenge' && (
            <div className="w-full flex items-center justify-between px-2 lg:px-8">
              <div className="flex flex-col items-start">
                <span className="text-[8px] md:text-[10px] font-black text-neutral-400 uppercase tracking-widest mb-1">Level</span>
                <div className="text-sm md:text-xl lg:text-2xl font-black text-primary bg-primary/5/50 px-3 md:px-5 py-1.5 md:py-2 rounded-xl">
                  {level}
                </div>
              </div>
              <div className="flex flex-col items-center">
                <span className="text-[8px] md:text-[10px] font-black text-neutral-400 uppercase tracking-widest mb-1">Points</span>
                <div className="text-sm md:text-xl lg:text-2xl font-black text-primary bg-primary/5/50 px-3 md:px-5 py-1.5 md:py-2 rounded-xl tabular-nums">
                  {points}
                </div>
              </div>
              <div className="flex flex-col items-end">
                <span className="text-[8px] md:text-[10px] font-black text-neutral-400 uppercase tracking-widest mb-1">High Score</span>
                <div className="text-sm md:text-xl lg:text-2xl font-black text-amber-500 bg-warning-bg/50 px-3 md:px-5 py-1.5 md:py-2 rounded-xl tabular-nums">
                  {highScore}
                </div>
              </div>
            </div>
          ...
          <div className="flex flex-col items-center gap-2 text-center">
            {mode === 'challenge' ? (
              <div className="flex flex-col items-center gap-1 md:gap-4">
                <span className="text-[10px] md:text-xs font-black text-primary/70 uppercase tracking-[0.4em]">Challenge Mode</span>
                <span className="text-3xl md:text-7xl lg:text-8xl font-black text-primary uppercase tracking-tighter">
                  Question {questionNumber}
                </span>
              </div>
            ) : (
              <div className="flex flex-col items-center">
                <span className="text-[10px] md:text-xs font-black text-neutral-400 uppercase tracking-[0.4em] mb-2 md:mb-4">Practice Mode</span>
                <div className="text-xl md:text-4xl lg:text-5xl font-black text-primary/70 bg-primary/5 px-6 md:px-10 py-2 md:py-4 rounded-2xl md:rounded-[2.5rem] uppercase tracking-widest">
                  Explore Binary
                </div>
              </div>
            )}
                ...
                <div key={idx} className="flex flex-col items-center gap-2 md:gap-4">
                  ...
                  <motion.button
                    ...
                    className={`w-full aspect-[2/3] rounded-xl md:rounded-[2rem] border-2 md:border-4 flex flex-col items-center justify-center gap-1 md:gap-4 transition-all duration-500 ${bit === 1
                        ? 'bg-primary border-indigo-400 text-white shadow-lg shadow-indigo-500/20'
                        : 'bg-surface border-slate-50 text-slate-200 hover:border-primary/20'
        ...
        <div className={`w-full max-w-6xl ${isMobile ? 'flex-1 py-2' : 'flex-1 py-4'} flex items-center justify-center overflow-hidden`}>
            {gameState === 'playing' ? (
              <div
                className={`w-full h-full rounded-[2rem] lg:rounded-[2.5rem] px-4 py-6 lg:px-8 lg:py-6 border-4 flex flex-col items-center justify-center gap-6 lg:gap-16 transition-colors duration-300 ${isCorrect ? 'bg-success-bg/50 border-emerald-400' : 'bg-slate-50/50 border-white'}`}
              >
                {mode === 'challenge' ? (
                  <div className={`flex flex-col md:flex-row items-center gap-4 md:gap-8 lg:gap-12 ${isMobile ? 'w-full' : ''}`}>
                    <div className={`flex flex-col items-center gap-2 md:gap-4 ${isMobile ? 'w-full' : 'w-[320px] lg:w-[400px] flex-shrink-0'}`}>
                      <span className="text-xs md:text-sm font-black text-primary uppercase tracking-[0.2em]">Decimal Number</span>
                      <div className={`bg-surface text-5xl md:text-8xl lg:text-9xl font-black text-primary w-full ${isMobile ? 'h-[100px]' : 'h-[140px] lg:h-[200px]'} flex items-center justify-center rounded-2xl md:rounded-[2.5rem] tabular-nums shadow-sm border-none`}>
                    ...
                    <div className={`grid grid-cols-3 gap-2 md:gap-3 lg:gap-4 w-full md:w-max ${challengeType === 'toBinary' ? 'opacity-20 pointer-events-none' : ''}`}>
                      {[1, 2, 3, 4, 5, 6, 7, 8, 9].map((num) => (
                        <button
                          ...
                          className="w-full md:w-16 lg:w-20 h-12 md:h-16 lg:h-20 bg-surface hover:bg-primary/5 text-primary rounded-xl md:rounded-[1.2rem] font-black text-lg md:text-2xl shadow-sm transition-all active:scale-90 border-2 border-indigo-50/50"
                        >
                          {num}
                        </button>
                      ))}
                      <button
                        ...
                        className="w-full md:w-16 lg:w-20 h-12 md:h-16 lg:h-20 bg-caution-bg hover:bg-rose-100 text-caution rounded-xl md:rounded-[1.2rem] flex items-center justify-center shadow-sm transition-all active:scale-90"
                      >
                      ...
                      <button
                        onClick={() => {}}
                        className="w-full md:w-16 lg:w-20 h-12 md:h-16 lg:h-20 bg-primary hover:bg-primary/90 text-white rounded-xl md:rounded-[1.2rem] flex items-center justify-center shadow-sm transition-all active:scale-90"
                      >
                  ...
                  <div className="flex flex-wrap lg:flex-nowrap items-center justify-center gap-1 md:gap-2 min-h-[100px] lg:min-h-[160px] w-full overflow-x-auto no-scrollbar">
                    {binary.map((bit, i) => bit === 1 && (
                      <div key={i} className="flex items-center gap-1 md:gap-2 px-2 md:px-3 py-1 md:py-2 bg-primary text-white rounded-lg md:rounded-xl shadow-sm shrink-0">
                    ...
                    <div className="flex items-center gap-2 md:gap-4 px-2 md:px-4 py-2 md:py-4 shrink-0">
                      ...
                      <span className="text-4xl md:text-7xl lg:text-9xl font-black text-primary tabular-nums tracking-tighter leading-none min-w-[3ch] text-center">{decimal}</span>
                    </div>
                  </div>
                )}
              </div>
              ...
              >
                ...
                <button
                  onClick={resetTool}
                  className="px-12 py-3 bg-primary text-white rounded-2xl font-black uppercase tracking-widest hover:bg-primary/90 transition-all active:scale-95 shadow-xl shadow-indigo-500/20"
                >
                  Play Again
                </button>
              </motion.div>
