  ...
  return (
    <div className="tool-container flex flex-col lg:flex-row gap-8 h-full font-['Outfit'] select-none relative bg-white rounded-[4rem] p-4 lg:p-12 italic  overflow-hidden">
      ...
      <div className="flex-1 flex flex-col gap-8 min-h-0">
        <div className="flex-1 bg-slate-50/50 rounded-[4rem] border-4 border-white  flex flex-col items-center justify-center relative overflow-hidden group">
          ...
          >
            ...
            <div className="w-12 h-12 bg-white border-4 border-slate-200 rounded-[1.5rem]  group-hover/dragger:border-indigo-400 flex items-center justify-center transition-all group-hover/dragger:scale-110 group-hover/dragger:rotate-90">
              <Maximize2 size={20} strokeWidth={3} className="text-slate-400 group-hover/dragger:text-indigo-600" />
            </div>
            <input 
              type="range"
              min="200"
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {[
            { label: 'Speed', value: frequency, unit: 'Hz', color: 'text-indigo-600', icon: Waves },
            { label: 'Length', value: wavelength.toFixed(1), unit: 'px', color: 'text-emerald-600', icon: Activity },
            { label: 'Velocity', value: waveSpeed, unit: 'px/s', color: 'text-rose-600', icon: Zap },
            { label: 'Harmonic', value: (2 * stringLength / wavelength).toFixed(1), unit: 'n', color: 'text-slate-900', icon: Sliders },
          ].map((stat, i) => (
            <div key={i} className="bg-white p-6 rounded-[2.5rem] border-4 border-slate-50  flex items-center gap-5 italic group hover:border-indigo-100 transition-all">
      ...
      <div className="w-full lg:w-[450px] shrink-0 flex flex-col gap-8 relative z-20 italic">
        ...
        <div className="bg-slate-900 p-8 rounded-[4rem] border-4 border-slate-800  flex flex-col gap-4 relative overflow-hidden shrink-0">
              ...
              {[
                ...
                <button
                  ...
                  className={`flex flex-col items-center gap-3 p-6 rounded-[2.5rem] border-4 transition-all ${
                    toggle.active ? 'bg-indigo-600 border-indigo-400 text-white ' : 'bg-white/5 border-white/5 text-slate-500 hover:text-white hover:bg-white/10'
                  }`}
                >
           ...
           <div className="flex-1 flex flex-col gap-10 overflow-y-auto no-scrollbar pr-2 pb-4">
              ...
              <div className="space-y-4">
                <div className="flex justify-between items-center px-2">
                   ...
                   <span className="text-2xl font-black text-indigo-600 italic tabular-nums">{frequency} Hz</span>
                </div>
                <div className="h-6 bg-white rounded-full p-1 border-2 border-white relative group ">
                   <input 
              ...
              <div className="space-y-4">
                <div className="flex justify-between items-center px-2">
                   ...
                   <span className="text-2xl font-black text-emerald-600 italic tabular-nums">{amplitude} px</span>
                </div>
                <div className="h-6 bg-white rounded-full p-1 border-2 border-white relative group ">
                   <input 
              ...
              <div className="space-y-4">
                <div className="flex justify-between items-center px-2">
                   ...
                   <span className="text-2xl font-black text-rose-600 italic tabular-nums">{waveSpeed} px/s</span>
                </div>
                <div className="h-6 bg-white rounded-full p-1 border-2 border-white relative group ">
                   <input 
